...
export default class Mainpage extends React.Component {
  ...
  .then(res=> {
    var obj = {}
    res.map(item=> {
      obj[item["_id"]]={position: item.locations.position, yourshopname: item.locations.yourshopname, items: item.items}}
    )
            ...
            <Popup
              ...
              >
                <div className="shopname">
                  ...
                  <div className="item">
                    {this.state.station["items"].map(item=> <p>Name: {[item["name"]]}<br></br> Description: {item[["description"]]}<br></br> Value: {item[["value"]]}<button>Request</button></p>                )}
                  </div>
                  <div id="item">
